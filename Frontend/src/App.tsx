...
function App() {
  ...
  useEffect(() => {
    if (selectedRepo && !selectedRepo?.commits) {
      setIsLoading(true)
      axios.get(selectedRepo.url + '/commits').then(res => {
        setIsLoading(false)
        setSelectedRepo(prev => ({
          ...prev,
          commits: res.data.map((commit_data: CommitData) => ({
          ...
          {selectedRepo ?
            <React.Fragment>
              <button
                ...
                onClick={() => setSelectedRepo(undefined)}>🔙 Back</button>
              <div className='border p-2 max-w-[50vw] rounded-md cursor-pointer'
              >
                <h3><span className='font-bold'>Name:</span> {selectedRepo.name}</h3>
                <p><span className='font-bold'>Desription:</span> {selectedRepo.description}</p>
                <p>
                  <span className='font-bold'>Language:</span> {selectedRepo.language}
            ...
            <React.Fragment>
              {data
                ...
                ?.map((repo, index) => {
                  return (
                    <div key={index} className='border p-2 max-w-[50vw] rounded-md cursor-pointer hover:scale-105 transition-all'
                      onClick={() => setSelectedRepo(repo)}
                    >
                      <h3><span className='font-bold'>Name:</span> {repo.name}</h3>
                      <p><span className='font-bold'>Desription:</span> {repo.description}</p>
                      <p><span className='font-bold'>Language:</span> {repo.language}
